...
\label{sec:k_ytt_kohteet}
  ...
  \label{sub:klusterointi}
    ...
    \end{itemize}
    \subsubsection{Nelikkomenetelmä} % (fold)
  ...
  \subsection{Kuvantunnistus} % (fold)
  \label{sub:kuvantunnistus}

  % subsection kuvantunnistus (end)
...
\label{sec:muita_samankaltaisuuden_metriikoita}
  ...
  \label{sub:google_similarity_distance}
    ...
    % subsubsection kaytannon_esimerkki (end)

    \subsubsection{Okeitus hakukoneiden käytölle samankaltiasuudessa} % (fold) % TODO: Otsikko
    \label{ssub:theory_of_googling_for_similarity}

    % TODO: number of webpages indexed by google

    Googlen indeksöityjen internetsivujen määrä on kasvanut suunnattomaksi ja mikä tahansa yleinen hakutermi esintyy miljoonilla internetsivulla. Internetin sisällöntuottajiakin on suunnaton määrä ja voidaan olettaa näiden olevan erittäin kuvaava otanta isosta osasta ihmiskuntaa. Tämän perusteella uskomme että, Google-haun frekvenssi on kuvaava approksimaatio todellisista relatiivisistä frekvensseistä yhteiskunnassa. Haun tulos on frekvenssi haun tuottamista osumista ja Googlen indeksoitujen sivujen lukumäärästä. \cite{cilibrasi2007google}

    % subsubsection theory_of_googling_for_similarity (end)
    \subsubsection{Google-jakauma} % (fold)
    \label{ssub:google_jakauma}
      Olkoon yksittäisten Google-hakujen joukko $S$. Jatkossa käytämme yksittäisten ja pareittaisten hakujen joukkoja $\{\{x,y\}:x,y \in S\}$. Olkoon joukko Googlen indeksöimiä internetsivuja $\Omega$. Joukon $\Omega$ koko merkitään $M = |\Omega|$.
      Oletamme että internetsivut ovat tasajakauteuneita, eli todennäköisyys että se palautuu Google-hausta on $\frac{1}{\Omega}$. $\Omega$:n alijoukkoa kutsutaan \emph{tapahtumaksi} ja määrittelemme jokaisen hakuehdon olevan tapahtuma $x$, $x \subseteq \Omega$
    % subsubsection google_jakauma (end)

    \subsubsection{Google-koodi} % (fold)
    \label{ssub:google_koodi}
      ...
      Määrittelemme
      \begin{align}
        g(x) = g(x,x),\, g(x,y) = L(x \cap y)M/N = |x\cap y|/N.
      \end{align}
    % subsubsection google_koodi (end)
...
\begin{itemize}
  \item kohinansieto
  \item pakkaajan valinta
\end{itemize}

\paragraph{Muita klusteroinnin menetelmiä} % (fold)
\label{par:muita_klusteroinnin_menetelmia}
  \begin{itemize}
    \item Hieraskinen klusterointi % TODO: http://en.wikipedia.org/wiki/Hierarchical_clustering
    \item k-means klusterointi % TODO: http://en.wikipedia.org/wiki/K-means_clustering
    \item Muita: \url{http://en.wikipedia.org/wiki/Data_clustering#Clustering_algorithms}
  \end{itemize}
