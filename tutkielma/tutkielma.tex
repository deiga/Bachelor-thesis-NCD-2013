...
\abstract{
Esittelemme metodin pakkauspohjaiseen klusterointiin. Tämä metodi ei ole riippuvainen kohteen ominaisuuksista tai taustatiedoista. Metodi toimii seuraavanlaisesti: Ensin määritämme parametrittoman, universaalin samankaltaisuusetäisyyden normalisoitu pakkausetäisyys (NCD), joka lasketaan pakatuista tiedostoista, yksittäin ja pareittain katenoituna. Seuraavaksi sovellamme hierarkkista klusterointimenetelmää. NCD ei ole rajattu tiettyyn sovellusalueeseen ja sitä voi käyttää useaan alueeseen kerralla. Esittelemme Kolmogorov-kompleksisuutta ja tähän perustuvan normalisoidun informaatioetäisyyden, jotka muodostavat NCD:n pohjan. Tämän lisäksi käymme läpi NCD:n kohinansietokykyä ja rajoitteuta pakkaajan valinnassa NCD:n käyttöä varten. Seuraavaksi esitellään Google-samankaltaisuusetäisyys, joka perustuu samaan toimintaperiaatteeseen kuin NCD, mutta käyttää internethakutuloksien lukumäärää samankaltaisuuden mittarina.
}
...
\label{par:intro-1}
  ...
  % TODO: clearer
  Käytetyt yhtälöt pätevät aina lähes täsmälleen, sillä siirrytään Kolmogorov-kompleksisuudesta laskettavaan approksimaatioon. Approksimaation virhe on hyvin pieni: $O(\log n)$ ($n$ on esimerkiksi tiedoston pituus).
...
\label{sec:yhteenveto}
  ...
  Nelikkomenetelmästä (kts. \ref{sub:nelikkomenetelma}) löytyy tarkempaa kuvausta artikkelissa \cite{cilibrasi2011fast}.
